...
class Parser(object):
  ...
  def format(self):
    ...
    for row in self.data:
      ...
      if (port!=None):
        print('<td><a href="http://supayr.ninja:{1}" target="_blank">{0}</a></td>'.format(row[0],port))
      else:
      ...
      print('    <input type="text" name="newPort" value="{0}" onfocus="this.style.border=\'1px solid lightgrey\'; this.style.background=\'white\'; this.value=this.form.oldPort.value" onblur="this.style.border=\'none\'; this.style.background=\'transparent\'; if (this.value!=this.form.oldName.value) this.form.submit()" >'.format(row[-2]))
