...
class Parser(object):
  ...
  def format(self):
    ...
    for row in self.data:
      ...
      print('    <input type="text" name="newPort" value="{0}" port="{1}" onfocus="this.style.border=\'1px solid lightgrey\'; this.style.background=\'white\'; this.value=this.getAttribute(\'port\')" onblur="this.style.border=\'none\'; this.style.background=\'transparent\'; if (this.value!=this.form.oldName.value) this.form.submit()" >'.format(row[-2],port if port!=None else ''))
      print('  </form>')
      print('</td>')
      print('<td>')
      print('<td>')
